        ...
        <div id="c_1" class="card border shadow-0 mb-g shadow-sm-hover" data-filter-tags="oliver kopyov">
            <div class="card-body border-faded border-top-0 border-left-0 border-right-0 rounded-top">
                <div class="d-flex flex-row align-items-center">
                    <span class="status status-{{ $user->status->slug }} mr-3">
                        <span class="rounded-circle profile-image d-block " style="background-image:url('{{ asset($user->avatar_path) }}'); background-size: cover;"></span>
                    </span>
                    <div class="info-card-text flex-1">
                        ...
                        <div class="dropdown-menu">
                            <a class="dropdown-item" href="{{ route('users.edit', $user->id) }}">
                                ...
                                Редактировать</a>
                            <a class="dropdown-item" href="{{ route('users.edit_credentials', $user->id) }}">
                                <i class="fa fa-lock"></i>
                                Безопасность</a>
                            <a class="dropdown-item" href="{{ route('users.edit_status', $user->id) }}">
                                <i class="fa fa-sun"></i>
                                Установить статус</a>
                            <a class="dropdown-item" href="{{ route('users.edit_avatar', $user->id) }}">
                                <i class="fa fa-camera"></i>
                                Загрузить аватар
                            </a>

                            <form method="POST" action="{{ route('users.destroy', $user->id) }}" style="margin-bottom: 0;">
                                @csrf
                                @method('DELETE')
